...
import { CHAT_MENU_ITEMS, getAnswerByMenuId, WELCOME_MESSAGE } from './config'
...
export function ChatPage() {
  ...
  // 處理目錄項目選擇
  const handleMenuSelect = useCallback((item: ChatMenuItem) => {
    // 添加用戶選擇消息
  ...
  return (
    <div className='flex h-[calc(100svh-var(--mobile-bar-full-height))] flex-col bg-blue-50/30'>
      {/* 頂部標題欄 */}
      <div className='flex items-center gap-3 border-grey-200 border-b bg-white px-4 py-3 shadow-sm'>
        <button
          onClick={() => router.back()}
        ...
        <div className='flex items-center gap-2'>
          ...
          <div>
            <h1 className='font-semibold text-foreground-primary'>
              減碳小精靈
            </h1>
      ...
      {/* 消息區域 */}
      <div className='flex-1 overflow-y-auto px-4 py-4 pr-5'>
        {messages.map((message) => (
          <ChatMessageBubble
            key={message.id}
            message={message}
            onBackToMenu={message.showBackToMenu ? handleBackToMenu : undefined}
          />
        ))}
