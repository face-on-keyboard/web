...
export function useRecords(options?: {
  ...
  const { data: originalRecords, isLoading: loading, error } = useInvoices()
  ...
  const records = useMemo(() => {
    ...
    return [
      ...
      ...(segments || []).map((segment) => ({
        id: segment.id,
        invoiceNumber: '0000',
        date: segment.fromTime,
        storeName: segment.travelMode,
        totalAmount: 0,
        category: '交通',
        totalCO2: segment.estimatedCO2,
        items: [],
      })),
    ]
  }, [originalRecords, segments])
