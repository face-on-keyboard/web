...
export const EarthStatus = ({ emissionValue }: EarthStatusProps) => {
	const tier = (EARTH_TIERS.find(({ max }) => emissionValue < max) ??
		EARTH_TIERS[EARTH_TIERS.length - 1]) as (typeof EARTH_TIERS)[number]

	return (
		<div className='flex flex-col items-center text-center'>
			<div
				className={`relative flex h-44 w-44 items-center justify-center ${tier.animation}`}
			>
				<Image
					src={tier.image}
					alt={`${tier.title} 地球`}
			...
			<div className='mt-3 font-semibold text-foreground-primary text-sm'>
				{tier.title}
			</div>
			<div className='mt-1 text-foreground-muted text-xs'>{tier.message}</div>
			<div className='mt-2 rounded-full bg-grey-100 px-3 py-1 font-medium text-[11px] text-foreground-secondary'>
