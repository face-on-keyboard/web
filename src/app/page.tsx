...
export default function HomePage() {
  ...
  const totalCO2 = filteredRecords.reduce(
    (sum, record) => sum + record.totalCO2,
    0
  )
  ...
  return (
    <main className='min-h-screen bg-background-muted px-3 py-4 sm:px-4 sm:py-6 md:py-8'>
      {/* 手機布局切換按鈕（僅在桌面顯示） */}
      <div className='fixed top-4 right-4 z-50 hidden md:block'>
        <button
          onClick={() => setMobileView(!mobileView)}
          className='rounded-lg bg-primary-600 px-4 py-2 font-semibold text-sm text-white shadow-lg transition-colors hover:bg-primary-700'
      ...
      <div className={`mx-auto ${mobileView ? 'max-w-sm' : 'max-w-7xl'}`}>
        ...
        <div className={`mb-4 ${mobileView ? '' : 'sm:mb-6 md:mb-8'}`}>
          <h1
            className={`mb-1 font-semibold text-2xl text-foreground-primary ${
              mobileView ? '' : 'sm:mb-2 sm:text-3xl md:text-h1'
            }`}
          >
            碳排 Dashboard
          </h1>
          <p
            className={`text-foreground-secondary text-sm ${
              mobileView ? '' : 'sm:text-body'
            }`}
          >
            從統一發票自動追蹤和分析您的碳排放量
          </p>
          {mobileView && (
            <div className='mt-2 rounded-lg bg-primary-50 p-2 text-primary-700 text-xs'>
              📱 手機布局預覽模式
            </div>
          )}
        </div>
        ...
        {/* 本月碳排對比區塊 */}
        <div
          className={`mb-4 rounded-lg bg-gradient-to-r from-primary-500 to-primary-600 p-4 text-white shadow-lg ${
            mobileView ? '' : 'sm:mb-6 sm:p-6'
          }`}
        >
          <div className='mb-3 flex items-center justify-between sm:mb-4'>
            <div>
              <div
                className={`mb-1 text-xs opacity-90 ${
                  mobileView ? '' : 'sm:text-sm'
                }`}
              >
                本月碳排放
              </div>
              <div
                className={`font-bold text-2xl ${
                  mobileView ? '' : 'sm:text-3xl md:text-4xl'
                }`}
              >
                ...
                <>
                  ...
                  <div
                    className={`flex items-center gap-1 font-bold text-lg ${
                      mobileView ? '' : 'sm:text-xl md:text-2xl'
                    }`}
                  >
          ...
          {monthlyStats.lastMonth > 0 && (
            <div
              className={`mt-3 rounded-lg bg-white/20 p-2 text-xs backdrop-blur-sm ${
                mobileView ? '' : 'sm:text-sm'
              }`}
            >
        ...
        {/* 錯誤訊息 */}
        {error && (
          <div className='mb-4 rounded-lg border border-red-300 bg-red-50 p-3 text-red-600 text-sm sm:mb-6 sm:p-4 sm:text-body'>
            ⚠️ {error}
          </div>
        )}
        ...
        >
          ...
          >
            <div
              className={`mb-1 text-foreground-muted text-xs ${
                mobileView ? '' : 'sm:mb-2 sm:text-caption'
              }`}
            >
              總碳排放量
            </div>
            <div
              className={`font-semibold text-primary-600 text-xl ${
                mobileView ? '' : 'sm:text-2xl md:text-h1'
              }`}
            >
              {totalCO2.toFixed(2)}
            </div>
        ...
        {Object.keys(categoryStats).length > 0 && (
          ...
          >
            <h2
              className={`mb-3 font-semibold text-foreground-primary text-lg ${
                mobileView ? '' : 'sm:mb-4 sm:text-xl md:text-h2'
              }`}
            >
              碳排組成
            </h2>
                ...
                .map(([category, amount]) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className='flex items-center gap-2'>
                        <div
                          className={`h-2 flex-1 rounded-full bg-grey-200 ${
                            mobileView
                              ? ''
                              : 'sm:min-w-[120px] md:min-w-[200px]'
                          }`}
                        >
                          <div
                        ...
                        <span
                          className={`font-semibold text-foreground-primary text-xs ${
                            mobileView ? '' : 'sm:text-sm md:text-body'
                          }`}
                        >
                          {amount.toFixed(2)} kg
                        </span>
        ...
        >
          ...
          >
            <h2
              className={`font-semibold text-foreground-primary text-lg ${
                mobileView ? '' : 'sm:text-xl md:text-h2'
              }`}
            >
              記錄列表
            </h2>
          ...
          {loading && records.length === 0 ? (
            <div className={`py-8 text-center ${mobileView ? '' : 'sm:py-12'}`}>
              <div
                className={`mb-3 animate-pulse text-3xl ${
                  mobileView ? '' : 'sm:mb-4 sm:text-4xl'
                }`}
              >
                📄
              </div>
            ...
            <div className={`py-8 text-center ${mobileView ? '' : 'sm:py-12'}`}>
              ...
              {hasActiveFilters && (
                <button
                  ...
                  className={`mt-3 text-primary-600 text-sm hover:text-primary-700 ${
                    mobileView ? '' : 'sm:mt-4 sm:text-body'
                  }`}
                >
                  清除過濾條件
                </button>
              ...
              {filteredRecords.map((record) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <div
                        className={`flex items-start gap-3 ${
                          mobileView ? '' : 'sm:items-center sm:gap-4'
                        }`}
                      >
                        ...
                        <div className='min-w-0 flex-1'>
                          <div
                            className={`mb-1.5 flex flex-wrap items-center gap-1.5 ${
                              mobileView ? '' : 'sm:mb-1 sm:gap-2'
                            }`}
                          >
                            <span
                              className={`break-words font-semibold text-foreground-primary text-sm ${
                                mobileView ? '' : 'sm:text-body'
                              }`}
                            >
                              {record.storeName}
                            ...
                            <span
                              className={`rounded-full px-1.5 py-0.5 text-xs ${
                                mobileView ? '' : 'sm:px-2 sm:text-caption'
                          ...
                          <div
                            className={`flex flex-wrap items-center gap-1.5 text-foreground-muted text-xs ${
                              mobileView ? '' : 'sm:gap-2 sm:text-caption'
                            }`}
                          >
                      ...
                      >
                        <div
                          className={`font-semibold text-base text-primary-600 ${
                            mobileView ? '' : 'sm:text-lg md:text-h3'
                          }`}
                        >
                    ...
                    {/* 展開的明細 */}
                    {isExpanded && (
                      <div className='border-grey-200 border-t bg-grey-50 p-3 sm:p-4'>
                        <div
                          className={`mb-2 font-semibold text-foreground-primary text-xs ${
                            mobileView ? '' : 'sm:text-sm'
                          }`}
                        >
                          商品明細
                          ...
                          {record.items.map((item, index) => (
                            ...
                            >
                              ...
                              <div className='ml-3 text-right'>
                                <div
                                  className={`font-semibold text-primary-600 text-xs ${
                                    mobileView ? '' : 'sm:text-sm'
                                  }`}
                                >
                        ...
                        <div
                          className={`mt-3 flex items-center justify-between rounded-lg bg-primary-50 p-2 font-semibold text-primary-700 text-xs ${
                            mobileView ? '' : 'sm:text-sm'
                          }`}
                        >
                          <span>總計</span>
        ...
        >
          ...
          >
            ...
            {hasActiveFilters && (
              <button
                onClick={clearFilters}
                className='self-start text-primary-600 text-xs hover:text-primary-700 sm:text-caption'
              >
                清除所有過濾
              </button>
            )}
          ...
          <div
            className={`grid gap-3 ${
              mobileView ? '' : 'sm:grid-cols-2 sm:gap-4 lg:grid-cols-4'
            }`}
          >
            {/* 搜索框 */}
            <div className={mobileView ? '' : 'sm:col-span-2 lg:col-span-2'}>
              <label
                className={`mb-1.5 block font-semibold text-foreground-primary text-xs ${
                  mobileView ? '' : 'sm:mb-2 sm:text-caption'
                }`}
              >
                搜尋商品/服務
              ...
              <div className='relative'>
                <input
                  ...
                  className={`w-full rounded-lg border border-grey-300 px-3 py-2.5 pl-9 text-sm focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-200 ${
                    mobileView ? '' : 'sm:px-4 sm:pl-10 sm:text-body'
                  }`}
                />
                <span
                  className={`-translate-y-1/2 absolute top-1/2 left-2.5 text-foreground-muted ${
                    mobileView ? '' : 'sm:left-3'
                  }`}
                >
                  🔍
                </span>
            ...
            <div>
              ...
              <select
                ...
                className={`w-full rounded-lg border border-grey-300 px-3 py-2.5 text-sm focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-200 ${
                  mobileView ? '' : 'sm:px-4 sm:text-body'
                }`}
              >
          ...
          <div
            className={`mt-3 grid gap-3 ${
              mobileView ? '' : 'sm:mt-4 sm:grid-cols-3 sm:gap-4'
            }`}
          >
            <div>
              <label
            ...
            <div className='flex items-end'>
              <button
                ...
                disabled={loading}
                className={`w-full rounded-lg bg-primary-600 px-4 py-2.5 font-semibold text-sm text-white transition-colors hover:bg-primary-700 disabled:cursor-not-allowed disabled:bg-grey-400 ${
                  mobileView ? '' : 'sm:px-6 sm:text-body'
                }`}
              >
                {loading ? '載入中...' : '🔄 重新載入'}
              </button>
          ...
          {hasActiveFilters && (
            <div
              className={`mt-3 rounded-lg bg-primary-50 p-2.5 text-primary-700 text-xs ${
                mobileView ? '' : 'sm:mt-4 sm:p-3 sm:text-caption'
              }`}
            >
